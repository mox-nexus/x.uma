...
| — (new) | **puma and bumi have no xDS types generated.** `puma/proto/src/gen/` and `bumi/proto/src/gen/` hold only `xuma`; `buf.gen.rust.yaml`'s second pass outputs Rust alone. Both need betterproto and ts-proto outputs added there before either can parse protojson. This is not a small step and nothing in Phase C currently names it. |
