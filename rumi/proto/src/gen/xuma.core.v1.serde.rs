// @generated
impl serde::Serialize for BoolMatcher {
    #[allow(deprecated)]
        ...
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("xuma.core.v1.BoolMatcher", len)?;
        if self.expected {
            struct_ser.serialize_field("expected", &self.expected)?;
        }
        struct_ser.end()
    }
}
impl<'de> serde::Deserialize<'de> for BoolMatcher {
    #[allow(deprecated)]
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = BoolMatcher;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct xuma.core.v1.BoolMatcher")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<BoolMatcher, V::Error>
                where
                ...
                while let Some(k) = map_.next_key()? {
                    match k {
                        GeneratedField::Expected => {
                            ...
                            expected__ = Some(map_.next_value()?);
                        }
                    }
                }
                Ok(BoolMatcher {
                    expected: expected__.unwrap_or_default(),
                })
            }
        }
        deserializer.deserialize_struct("xuma.core.v1.BoolMatcher", FIELDS, GeneratedVisitor)
    }
}
