...
//! both PINNED in `buf.gen.yaml` — see the note there for why. Those pins also fix the
//! output layout: v0.4.0 emits flat `gen/<package>.rs` files, v0.5.0+ emits a
//! nested directory tree. If these `include!` paths ever stop resolving, check
//! whether a plugin version moved before changing them.
...
pub mod xds {
    pub mod core {
        pub mod v3 {
            include!("gen/xds.core.v3.rs");
        }
    }
    pub mod r#type {
        pub mod v3 {
            include!("gen/xds.type.v3.rs");
        }
        pub mod matcher {
            pub mod v3 {
                include!("gen/xds.type.matcher.v3.rs");
            }
        }
    }
}
...
pub mod xuma {
    pub mod core {
        pub mod v1 {
            include!("gen/xuma.core.v1.rs");
        }
    }
    pub mod http {
        pub mod v1 {
            include!("gen/xuma.http.v1.rs");
        }
    }
    pub mod claude {
        pub mod v1 {
